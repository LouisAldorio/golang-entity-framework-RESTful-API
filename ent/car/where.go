...
// HasCarUtilities applies the HasEdge predicate on the "car_utilities" edge.
func HasCarUtilities() predicate.Car {
	return predicate.Car(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(CarUtilitiesTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2M, false, CarUtilitiesTable, CarUtilitiesPrimaryKey...),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasCarUtilitiesWith applies the HasEdge predicate on the "car_utilities" edge with a given conditions (other predicates).
func HasCarUtilitiesWith(preds ...predicate.CarUtility) predicate.Car {
	return predicate.Car(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(CarUtilitiesInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2M, false, CarUtilitiesTable, CarUtilitiesPrimaryKey...),
		)
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
