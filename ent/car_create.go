...
func (cc *CarCreate) AddCarUtilityIDs(ids ...int) *CarCreate {
	cc.mutation.AddCarUtilityIDs(ids...)
	return cc
}

// AddCarUtilities adds the "car_utilities" edges to the CarUtility entity.
func (cc *CarCreate) AddCarUtilities(c ...*CarUtility) *CarCreate {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return cc.AddCarUtilityIDs(ids...)
}
...
func (cc *CarCreate) createSpec() (*Car, *sqlgraph.CreateSpec) {
	...
	if nodes := cc.mutation.CarUtilitiesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
