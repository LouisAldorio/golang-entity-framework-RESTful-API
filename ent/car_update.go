...
func (cu *CarUpdate) AddCarUtilityIDs(ids ...int) *CarUpdate {
	cu.mutation.AddCarUtilityIDs(ids...)
	return cu
}

// AddCarUtilities adds the "car_utilities" edges to the CarUtility entity.
func (cu *CarUpdate) AddCarUtilities(c ...*CarUtility) *CarUpdate {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return cu.AddCarUtilityIDs(ids...)
}
...
// ClearCarUtilities clears all "car_utilities" edges to the CarUtility entity.
func (cu *CarUpdate) ClearCarUtilities() *CarUpdate {
	cu.mutation.ClearCarUtilities()
	return cu
}

// RemoveCarUtilityIDs removes the "car_utilities" edge to CarUtility entities by IDs.
func (cu *CarUpdate) RemoveCarUtilityIDs(ids ...int) *CarUpdate {
	cu.mutation.RemoveCarUtilityIDs(ids...)
	return cu
}

// RemoveCarUtilities removes "car_utilities" edges to CarUtility entities.
func (cu *CarUpdate) RemoveCarUtilities(c ...*CarUtility) *CarUpdate {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return cu.RemoveCarUtilityIDs(ids...)
}
...
func (cu *CarUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if cu.mutation.CarUtilitiesCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
			Table:   car.CarUtilitiesTable,
			Columns: car.CarUtilitiesPrimaryKey,
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: &sqlgraph.FieldSpec{
					Type:   field.TypeInt,
	...
	if nodes := cu.mutation.RemovedCarUtilitiesIDs(); len(nodes) > 0 && !cu.mutation.CarUtilitiesCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
			Table:   car.CarUtilitiesTable,
	...
	if nodes := cu.mutation.CarUtilitiesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
			Table:   car.CarUtilitiesTable,
...
// AddCarUtilityIDs adds the "car_utilities" edge to the CarUtility entity by IDs.
func (cuo *CarUpdateOne) AddCarUtilityIDs(ids ...int) *CarUpdateOne {
	cuo.mutation.AddCarUtilityIDs(ids...)
	return cuo
}

// AddCarUtilities adds the "car_utilities" edges to the CarUtility entity.
func (cuo *CarUpdateOne) AddCarUtilities(c ...*CarUtility) *CarUpdateOne {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return cuo.AddCarUtilityIDs(ids...)
}
...
// ClearCarUtilities clears all "car_utilities" edges to the CarUtility entity.
func (cuo *CarUpdateOne) ClearCarUtilities() *CarUpdateOne {
	cuo.mutation.ClearCarUtilities()
	return cuo
}

// RemoveCarUtilityIDs removes the "car_utilities" edge to CarUtility entities by IDs.
func (cuo *CarUpdateOne) RemoveCarUtilityIDs(ids ...int) *CarUpdateOne {
	cuo.mutation.RemoveCarUtilityIDs(ids...)
	return cuo
}

// RemoveCarUtilities removes "car_utilities" edges to CarUtility entities.
func (cuo *CarUpdateOne) RemoveCarUtilities(c ...*CarUtility) *CarUpdateOne {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return cuo.RemoveCarUtilityIDs(ids...)
}
...
func (cuo *CarUpdateOne) sqlSave(ctx context.Context) (_node *Car, err error) {
	...
	if cuo.mutation.CarUtilitiesCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
			Table:   car.CarUtilitiesTable,
	...
	if nodes := cuo.mutation.RemovedCarUtilitiesIDs(); len(nodes) > 0 && !cuo.mutation.CarUtilitiesCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
			Table:   car.CarUtilitiesTable,
	...
	if nodes := cuo.mutation.CarUtilitiesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
			Table:   car.CarUtilitiesTable,
