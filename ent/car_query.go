...
type CarQuery struct {
	...
	// eager-loading edges.
	withOwner        *UserQuery
	withCarUtilities *CarUtilityQuery
	withFKs          bool
...
// QueryCarUtilities chains the current query on the "car_utilities" edge.
func (cq *CarQuery) QueryCarUtilities() *CarUtilityQuery {
	query := &CarUtilityQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CarQuery) Clone() *CarQuery {
	...
	return &CarQuery{
		...
		withCarUtilities: cq.withCarUtilities.Clone(),
...
// WithCarUtilities tells the query-builder to eager-load the nodes that are connected to
// the "car_utilities" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CarQuery) WithCarUtilities(opts ...func(*CarUtilityQuery)) *CarQuery {
	query := &CarUtilityQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withCarUtilities = query
	return cq
}
...
func (cq *CarQuery) sqlAll(ctx context.Context) ([]*Car, error) {
	...
	if query := cq.withCarUtilities; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Car, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.CarUtilities = []*CarUtility{}
		}
		var (
			edgeids []int
			edges   = make(map[int][]*Car)
		)
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(car.CarUtilitiesPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, cq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "car_utilities": %w`, err)
		}
		query.Where(carutility.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "car_utilities" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.CarUtilities = append(nodes[i].Edges.CarUtilities, n)
			}
		}
	}
